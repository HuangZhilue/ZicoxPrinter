...
public partial class BluetoothHelperViewModel : BaseViewModel
{
    ...
    public BluetoothHelperViewModel()
    {
        ...
        {
            if (
                sender is AppShell shell
                && shell.CurrentPage.BindingContext.GetType() == typeof(BluetoothHelperViewModel)
            )
            {
                NavigatedTo();
            }
...
#endif
            {
                ...
                Task.Run(async () =>
                {
                    ...
                    while (startTime.AddSeconds(12) > DateTime.Now)
                    {
                        ...
                        {
                            if (
                                !NotBondedDevices.Any(d =>
                                    d.Name == device.Key && d.Mac == device.Value
                                )
                            )
                        ...
                        Application.Current!.Dispatcher.Dispatch(() =>
                        {
                            NotBondedDevices.Add(
                                new("test" + DateTime.Now.ToString("ssff"), "12:34:56:78:90:AB")
                            );
                        });
#endif
